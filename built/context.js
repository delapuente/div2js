...
Context.prototype = {
    ...
    newAux: function (name, initializer) {
        ...
        var declaration = new ast.VariableDeclaration(new ast.VariableDeclarator(identifier, initializer));
        return {
...
Linearization.prototype = {
    ...
    _goToIf: function (testAst, labelIfTrue, labelIfFalse) {
        ...
        return {
            type: 'GoToIf',
            get sentences() {
                return [_this._programCounterBranch(testAst, labelIfTrue.label, labelIfFalse.label), new ast.BreakStatement()];
            }
        };
    },
        ...
        return {
            ...
            get sentences() {
                ...
                var cases = options.map(function (option) {
                    ...
                    return _this._programCounterBranch(t.some(evaluation, tests), option.label.label);
                });
    ...
    _programCounterBranch: function (testAst, consequent, alternate) {
        return new ast.ExpressionStatement(new ast.AssignmentExpression(t.programCounter, new ast.ConditionalExpression(testAst, new ast.Literal(consequent + 1), alternate ? new ast.Literal(alternate + 1) : t.programCounter)));
    },
    _programCounterSet: function (label) {
        return new ast.ExpressionStatement(new ast.AssignmentExpression(t.programCounter, new ast.Literal(label + 1)));
    },
