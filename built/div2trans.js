...
translators.AssignmentExpression = function (divAssignment, context) {
    return new ast.AssignmentExpression(translate(divAssignment.left, context), translate(divAssignment.right, context), 
...
translators.BinaryExpression = function (divBinary, context) {
    return new ast.BinaryExpression(translate(divBinary.left, context), translate(divBinary.right, context), divBinary.operator);
};
...
translators.CallExpression = function (divCall, context) {
    var parameters = new ast.ArrayExpression(divCall.arguments.map(function (arg) {
...
translators.Unit = function (divUnit, context) {
    ...
    return new ast.Program([globals, locals, privateOffset]
...
function createPrivateOffset(context) {
    ...
    return new ast.VariableDeclaration([
        new ast.VariableDeclarator(new ast.Identifier('P_OFFSET'), ast.Literal['for'](mmap.localSegmentSize))
    ]);
}
function getGlobalBaseDeclaration(context) {
    var offset = context.getMemoryMap().constructor.GLOBAL_OFFSET;
    return new ast.VariableDeclarator(t.globalBaseIdentifier, ast.Literal['for'](offset));
}
...
function getSegmentDeclarations(segment, prefixes, cells) {
    ...
    for (var i = 0, cell; (cell = cells[i]); i++) {
        if (!cell.hidden) {
            ...
            definitions.push(new ast.VariableDeclarator(t[identifierFactory](prefixes), ast.Literal['for'](cell.offset)));
            if (cell.type === 'struct') {
...
function translateForLikeLoop(loop, inits, tests, updates, context) {
    ...
    function verbatim(divExpression) {
        context.verbatim(new ast.ExpressionStatement(translate(divExpression, context)));
    }
}
