...
export default {
    ...
    _globalAddress: function (name) {
        return new ast.BinaryExpression(this.globalBaseIdentifier, this.identifierForGlobal(name), '+');
    },
    // XXX: Returns ast for `exec.base + L_<NAME>`
    _localAddress: function (name) {
        return new ast.BinaryExpression(this._localBase, this.identifierForLocal(name), '+');
    },
    // XXX: Returns ast for `exec.base + P_OFFSET + <name>`
    _privateAddress: function (name) {
        return new ast.BinaryExpression(new ast.BinaryExpression(this._localBase, this.privateOffsetIdentifier, '+'), new ast.Identifier(name), '+');
    },
    ...
    _localBase: new ast.MemberExpression(new ast.Identifier('exec'), new ast.Identifier('base'), false),
    ...
    call: function (kind, resume, name, argList) {
        ...
        return new ast.ReturnStatement(this.callWith(yieldType, [ast.Literal.for(resume), ast.Literal.for(name)].concat(argList)));
    },
    processClone: function (child, parent) {
        return new ast.ReturnStatement(this.callWith('__yieldClone', [ast.Literal.for(child), ast.Literal.for(parent)]));
    },
    ...
    processFrame: function (resume, expression) {
        return new ast.ReturnStatement(this.callWith('__yieldFrame', [ast.Literal.for(resume), expression]));
    },
    // TODO: Same here.
    processDebug: function (resume) {
        return new ast.ReturnStatement(this.callWith('__yieldDebug', [ast.Literal.for(resume)]));
    },
    programFunction: function (name, body) {
        return new ast.FunctionDeclaration(new ast.Identifier('program_' + name), this.processParameters, undefined, body);
    },
    processFunction: function (name, body) {
        return new ast.FunctionDeclaration(new ast.Identifier('process_' + name), this.processParameters, undefined, body);
    },
    ...
    get programCounter() {
        return new ast.MemberExpression(new ast.Identifier('exec'), new ast.Identifier('pc'));
    },
    get dequeueReturnValue() {
        return new ast.CallExpression(new ast.MemberExpression(new ast.MemberExpression(new ast.Identifier('exec'), new ast.Identifier('retv')), new ast.Identifier('dequeue')));
    },
